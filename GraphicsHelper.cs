...
namespace Utility {
  public class GraphicsHelper {
    public static void ReDraw(int exception = 0) {
      ...
      if (Game.player.state == "room" && Game.player.currentRoom != null) {

        
        if (Game.player.currentRoom.currentClue != "") {

          descriptionFromClue();
        }
        else {
          descriptionFromFile(0, Game.player.currentLocation.alias, Game.player.currentRoom.alias);
        }

      }
      else if (Game.player.state == "place") {
        descriptionFromFile(0, Game.player.currentLocation.alias, Game.player.currentLocation.alias);
      }
    ...
    public static void graphicFromFile(int type, string fileName) {
      ...
      path = fileName;
      file = FileHelper.loadGraphic(type, path.Replace(" ", string.Empty));
    ...
    public static void descriptionFromFile(int type, string dirName, string fileName) {

      List<string> file;

      

      file = FileHelper.loadDescription(type, dirName.Replace(" ", string.Empty), fileName.Replace(" ", string.Empty));
    ...
    public static void descriptionFromClue() {

      string clue;

      if (Game.player.currentRoom != null) {
        clue = Game.player.currentRoom.currentClue;
        Console.SetCursorPosition(2, 25);
        Console.WriteLine(clue);

        Game.player.currentRoom.currentClue = "";
      }
